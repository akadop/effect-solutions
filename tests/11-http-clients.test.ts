...
class GitHubApi extends Context.Tag("GitHubApi")<
  GitHubApi,
  {
    ...
    readonly listRepos: (
      username: string,
    ) => Effect.Effect<ReadonlyArray<Repo>>;
  }
>() {
  static layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const getUser = Effect.fn("GitHubApi.getUser")(function* (
        username: string,
      ) {
      ...
      const getRepo = Effect.fn("GitHubApi.getRepo")(function* (
        owner: string,
        repo: string,
      ) {
      ...
      const listRepos = Effect.fn("GitHubApi.listRepos")(function* (
        username: string,
      ) {
        const response = yield* client.get(`/users/${username}/repos`);
        return yield* HttpClientResponse.schemaBodyJson(Schema.Array(Repo))(
          response,
        );
      });
...
describe("HTTP Clients", () => {
    ...
    it.effect("fetch and decode a repository", () =>
      Effect.gen(function* () {
        const response = yield* HttpClient.get(
          "https://api.github.com/repos/Effect-TS/effect",
        );
  ...
  describe("Request Building", () => {
    ...
    it.effect("set query parameters", () =>
      Effect.gen(function* () {
        const request = HttpClientRequest.get(
          "https://api.github.com/users/effect-ts/repos",
        ).pipe(
        ...
        const response = yield* HttpClient.execute(request);
        const repos = yield* HttpClientResponse.schemaBodyJson(
          Schema.Array(Repo),
        )(response);
  ...
  describe("Response Handling", () => {
    ...
    it.effect("filterStatusOk fails on non-2xx", () =>
      Effect.gen(function* () {
        const response = yield* HttpClient.get(
          "https://api.github.com/repos/nonexistent-org-12345/nonexistent-repo",
        );
  ...
  describe("Error Handling", () => {
    it.effect("catches RequestError on network failure", () =>
      Effect.gen(function* () {
        const response = yield* HttpClient.get(
          "https://invalid.domain.that.does.not.exist.example/repos",
        );
        return yield* HttpClientResponse.schemaBodyJson(Repo)(response);
      }).pipe(
